  ...
  registerProps: ReturnType<UseFormRegister<object>>})
{
  return (
    <div className={'flex flex-col' + extraCss ? extraCss : ''}>
      {label && <label className={'text-sm mb-2'}><strong>{label}</strong></label>}
      <div className={`border-[2px] gap-2 flex flex-row items-center border-2 py-2 outline-none rounded-md ${iconClass ? 'pl-4 pr-2' : 'px-4'} ${error ? 'border-red-400' : 'border-[var(--color-border)]'}`}>
